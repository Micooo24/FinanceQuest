...
async def analyze_finances(user_id: str, year: int, month: int):
    ...
    first sentence includes the financial behaivour, Describe how the allocation on each finance category is in percentages, the Total Savings, Personal Savings, Remaining Budget, Total Income, Total Needs, Total Wants, and together with
    the percentage mention the actual value in money as well. Note I forgot to adjust the naming in database but the expenses is the needs and the bills is the wants so in your feedback use the correct naming no need to mention the expenses or bills JUST USE THE NEEDS OR WANTS
