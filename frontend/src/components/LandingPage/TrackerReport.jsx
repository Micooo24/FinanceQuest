...
const styles = StyleSheet.create({
  page: { padding: 30, position: "relative", fontFamily: "Roboto" , fontWeight: "bold" },
...
const TrackerReport = ({ totalIncome, totalExpenses, totalsavings, incomeData, expensesData, billsData, savingsData, aiAnalysis }) => {
  ...
  // Function to chunk data for tables (income, expenses, etc.)
  const chunkData = (data, rowsPerPage = 10) => {
    const chunks = [];
  ...
  // Watermark Component to be included in each chunk
  const Watermark = () => (
    ...
    <Document>
      {[...Array(2)].map((_, pageIndex) => (
        <Page key={pageIndex} style={styles.page}>
          ...
          {pageIndex === 0 && (
            <>
              {/* Breaking down the sections into chunks */}
              {[{ title: "Income Breakdown", data: incomeData }, { title: "Essential Expenses (Needs)", data: expensesData }, { title: "Discretionary Expenses (Wants)", data: billsData }, { title: "Personal Savings", data: savingsData }].map((section, idx) => {
                const chunks = chunkData(section.data, 20);  // Adjust rows per page as needed
                return chunks.map((chunk, chunkIndex) => (
                  <View key={`${idx}-${chunkIndex}`} style={styles.section} wrap={false}>
